...
The command examples use POSIX inline environment-variable syntax. In
PowerShell, set `$env:PLAYWRIGHT_HTML_OPEN = "never"` once before running the
same `npx` commands.
...
each so every test starts from a clean page. Parallelize only when fixtures,
accounts, server-side data, and session names are all explicitly isolated.
